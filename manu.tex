...
The objective of the third scenario, we want to see the peer requess effect to peer-assisted CDN when the request from peers are totally different from Youtube's videos popularity.
...
If $z_{dl} - z_{ms}=0$ then it same with PROP. 
...
We show that by introducing $z$ factor to utility function we can maintaining same peer contribution while reducing number of replicas.
