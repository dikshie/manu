...
\STATE ysearch $\leftarrow$ viewrate \COMMENT{view of a requested video}


\STATE $x1$ $\leftarrow$ $(xsearch - 1)$ \COMMENT{search the nearest point at one week before}
...
\FOR{$i=0$ to $len_2$}
    \IF{$tempy[i]$ $==$ $ysearch$}
    \STATE $result1[i]$ $\leftarrow$ $tempz[i]$ \COMMENT {get relative week only for ysearch}
    \ENDIF
\ENDFOR


\STATE $x1$ $\leftarrow$ $(xsearch)$ \COMMENT{search the nearest point at same week}
...
\STATE $len_2 \leftarrow length(tempy)$
\FOR{$i=0$ to $len_2$}
    \IF{$tempy[i]$ $==$ $ysearch$}
    \STATE $result2[i]$ $\leftarrow$ $tempz[i]$ \COMMENT {get relative week only for ysearch}
    \ENDIF
\ENDFOR
...
\STATE $x1$ $\leftarrow$ $(xsearch + 1)$ \COMMENT{search the nearest point at one week after}
...
\FOR{$i=0$ to $len_2$}
    \IF{$tempy[i]$ $==$ $ysearch$}
    \STATE $result3[i]$ $\leftarrow$ $tempz[i]$ \COMMENT {get relative week only for ysearch}
    \ENDIF
\ENDFOR
...
\STATE $n$ $\leftarrow$ $36$ \COMMENT{number of weeks}
\IF{$t$ $>$ $36$}
\STATE $phase$ $\leftarrow$ $after$
\ELSE
...
\FOR{$i=0$ to $t$}
\STATE $number$ $\leftarrow$ $count(temp,i)$ \COMMENT{count how many t's value inside temp}
\STATE $total$ $\leftarrow$ $total$ $+$ $count(temp,i)$ \COMMENT{add the number to total}
\ENDFOR
