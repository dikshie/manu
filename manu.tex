...
Denote $u_{dl}$ is utility function for a video inside the cache and $u_{ms}$ is utility function for a video that will enter the cache. 
...
\begin{align*}
(f(p_{dl}) - f(p_{min})) (f(p_{max}) - f(p_{dl})) - (f(p_{ms}) - f(p_{min})) (f(p_{max}) - f(p_{ms})) > \\
