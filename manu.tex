...
Moreover, Borghol et al., \cite{Borghol:2011:CMP:2039452.2039717} classify a video popularity become three categories: before-peak phase, at-peak phase, and after-peak phase.
...
The objective of estimating Internet VoD popularity phase is to get popularity state of a video whether a video is at before-peak popularity phase or at peak popularity phase, or at after-peak popularity phase.
...
We can estimate the that video phase by averaging relative week to peak numbers (the points at z-axis) of the nearest point from datasets. 
...
For example: there is a peer that requests a video where the position of video is in fourth week with the last week view rate $vr=2$ (we can get as this data from CDN) shown in fig.~\ref{fig:viewratedistexamplered} denote as cross.
...
Since we can estimate before-peak phase, at-peak phase, and after-peak phase of video, we modified the original utility function from PROP by adding a weight as follows:
\begin{equation}
...
\begin{equation}
p = min (\frac{n_i^r}{t_i^r - t_a^i}  , \frac{1}{t_{cur} - t_i^r})
\end{equation}
...
$z$ is proportion of view count that we get from Youtube datasets.  
...
Linear addition of $z$ factor can help to differentiate the value of utility function.
...
\hfill
\subfloat[Absolute contribution of peers for the third scenario where $y$-axis in log-scale.\label{fig:contrib-zipf}]{
...
Figure \ref{fig:atd-normal}, \ref{fig:atd-shift}, and \ref{fig:atd-normal} show the number of a video replicas available in system when a peer requests a video.
...
We can see the proportion of cached and not-cached event in table.\ref{tab:stacked1} and detail of the video phase breakdown is shown in table.\ref{tab:stacked2}.
In model, not-cached events take around $65\%$ from all events and majority of video phase is in after-peak for both cached events and not-cached events. 
Because the majority of video phase is in after-peak for both cached events and not-cached events, 
...
As we know from table.\ref{tab:stacked2} that the majority of a requested video is in after-peak phase and a requested video phase that is in at-peak phase is very small portion, then we can see that $z_{dl} - z_{ms}$ term will be in negative term if $z_{dl}$ is in before-peak phase or $0$ if $z_{dl}$ is in after-peak phase. 
Since the not-cached events happen in when a requested video phase in after-peak phase, we can get that $f(p_{ms}) - f(p_{dl} < 0$. 
...
We found that in the all scenarios, the model gives lower replica than PROP. 
This is because in the model, we found that not-cached events are higher than cached events. 
Therefore, in the model the number of available replicas are lower than PROP. 
