...
In before-peak and after-peak phases, we assume that requests to video are low thus we only consider minimum popularity.
We use eq.\ref{eq:utilityfunctionbeforeafter} for before-peak and after-peak phase.
In at-peak phase, we assume that request to video are high thus we only consider maximum popularity.   
We use eq.\ref{eq:utilityfunctionat} for at-peak phase.
...
Figure~\ref{fig:peercontribution} shows CDF peer contribution for our model compare to PROP. 
