...
Guo et al., \cite{1613869} work's PROP is closest with our work because we use that work as comparison and we use author's utility function.
...
In our work, we use Youtube VoD view model to aid our model that based from PROP. 
...
In order to evaluate the proposed peer-caching strategy using before-peak, at-peak, and after-peak information from VoD model, we have to compare our model to PROP model.
We evaluate two metrics which is peer contribution to delivery contents during simulation and access frequency of cache.
